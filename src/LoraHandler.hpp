...
void LoraHandler::lora_send(unsigned int &n) {
    if (LoRa.beginPacket()) {  // Check to make sure the packet was started successfully before writing data 
        LoRa.write(n);  // Write the data to the packet 
        LoRa.endPacket();  // End and send the packet 
    } else {   // If there was an error starting the packet, print an error message 
